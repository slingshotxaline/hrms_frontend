        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="border-t pt-4">
              <p className="text-sm font-semibold text-gray-500 uppercase mb-3">Allowances</p>
                  ...
                  {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(day => {
                    ...
                    return (
                      <tr key={day} className={isWeekend ? 'bg-red-50' : 'bg-white'}>
                        <td className="px-6 py-3 text-sm font-medium text-gray-900">{day}</td>
...
function SalaryRow({ label, amount, bold, highlight }) {
  return (
    <div className={`flex justify-between items-center ${highlight ? 'text-indigo-600' : ''}`}>
      <span className={`text-gray-700 ${bold ? 'font-bold text-lg' : ''}`}>{label}</span>
      <span className={`${bold ? 'font-bold text-xl text-indigo-600' : 'font-semibold text-gray-900'}`}>
